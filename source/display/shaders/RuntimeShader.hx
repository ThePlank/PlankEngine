//// I SWEAR TO GOD THIS DOSEN'T HAVE ANY CORELATION TO PSYCH ENGINE.
/// IT'S FROM THIS PULL REQUEST https://github.com/HaxeFlixel/flixel-addons/pull/368
...
class RuntimeShader extends FlxShader
{
	...
	private override function __initGL():Void
	{
		...
		@:privateAccess // this is the one time that this has to be used
	...
	/**
	 * Modify a buffer parameter of the shader.
	 * @param name The name of the parameter to modify.
	 * @param value The new value to use.
	 */
	public function setBuffer(name:String, value:RuntimeShader):Void
	{
		var prop:ShaderInput<BitmapData> = Reflect.field(this.data, name);
		if (prop == null)
		{
			trace('[WARN] Shader buffer property ${name} not found.');
			return;
		}
