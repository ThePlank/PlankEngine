...
class FlxInputText extends FlxText
{
			...
			BackgroundColor:Int = FlxColor.WHITE, EmbeddedFont:Bool = true)
	{
		...
		selectionSprite.makeGraphic(1, Std.int(size + 2), FlxColor.CYAN);
	...
	private function onKeyDown(e:KeyboardEvent):Void
	{
		...
		if (hasFocus)
		{
			...
			if (e.ctrlKey) {
				...
				// Stops anything from getting further
				return;
			}
	...
	private function getCharacterWidth(character:String, font:String, ?size:Int) {
		var fullPath:String = FileSystem.absolutePath(font);
		var fontBytes:Bytes = File.getBytes(fullPath);

		var limeFont:LimeFont = LimeFont.fromBytes(fontBytes);

		if (size != null)
			limeFont.unitsPerEM = size;
		else
			limeFont.unitsPerEM = 16;

		return Std.int(limeFont.getGlyphMetrics(limeFont.getGlyphs(character)[0]).verticalBearing.x);
	}

	private function getStringWidth(word:String, font:String, ?size:Int) {
		var seperatedCharacters:Array<String> = word.split("");
		var completeWidth:Int = 0;

		for (i in 0...seperatedCharacters.length) {
			completeWidth += getCharacterWidth(seperatedCharacters[i], font, size);
		}
