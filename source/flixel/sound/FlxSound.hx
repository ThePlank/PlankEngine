...
enum abstract FlxFilterType(Int) to Int {
	var LOWPASS = 0x0001;
	var HIGHPASS = 0x0002;
	var BANDPASS = 0x0003;
}

@:allow(states.AudioFilterTestState)
@:allow(flixel.sound.FlxSound)
class FlxFilter {
	@:isVar public var gain(default, set):Float;
	@:isVar public var filterGain(default, set):Float;
	public var type(default, set):FlxFilterType;
	var filter:ALFilter;

	public function new(type:FlxFilterType) {
		this.filter = AL.createFilter();
		this.type = type;
	}

	function set_type(type:FlxFilterType):FlxFilterType {
		AL.filteri(filter, AL.FILTER_TYPE, type);
		return this.type = type;
	}

	function set_gain(gain:Float):Float {
		AL.filterf(filter, 0x0001, gain);
		return this.gain = gain;
	}

	function set_filterGain(gain:Float):Float {
		AL.filterf(filter, 0x0002, gain);
		return this.filterGain = gain;
	}
}
...
class FlxSound extends FlxBasic
{
	...
	public var filter(default, set):FlxFilter;
	...
	/**
	 * Internal tracker for a OpenAL filter.
	 */
	...
	function applyFilter():Void {
		...
		AL.sourcei(_sound.__buffer.__srcBuffer, AL.DIRECT_FILTER, filter.filter);
		trace(filter.filter);
	}
	...
	inline function set_filter(filter:FlxFilter):FlxFilter {
		this.filter = filter;
		applyFilter();
		return filter;
	}
