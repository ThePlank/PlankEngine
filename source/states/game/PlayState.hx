...
class PlayState extends states.abstr.MusicBeatState
{
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				var sustainNote:Sustain = new Sustain(swagNote, Std.int(susLength));
	...
	override public function update(delta:Float)
	{
		...
		// uhm... why was this even in the game?
		// if (controls.CHEAT)
		// {
		// 	health += 1;
		...
		if (unspawnNotes[0] != null)
		{
			if (unspawnNotes[0].strumTime - Conductor.songPosition < 1500)
			{
				...
				if (dunceNote.mustPress)
					playerStrums.addNote(dunceNote);
				else
					dadStrums.addNote(dunceNote);
	...
	/*private function keyShit():Void
		{
			...
			if (generatedMusic)
				...
				{
					if (note.isSustainNote && note.canBeHit && note.mustPress && controlArrayHold[note.noteData])
						goodNoteHit(note);
				});
